...
if(isset($_POST['act'])) {
	if($_POST['act'] == '1') {
		...
		if(!isset($_POST['name']) || !isset($_POST['username']) || !isset($_POST['password1']) || !isset($_POST['password2']) || !isset($_POST['email']) || !isset($_POST['role']) || !isset($_POST['proyect']))
			die('wrong');
		if($_POST['name'] == '' || $_POST['username'] == '' || $_POST['password1'] == '' || $_POST['password2'] == '' || $_POST['email'] == '' || $_POST['role'] == ''  || $_POST['proyect'] == '')
			die('wrong');
		...
		if($_users->new_user($name, $username, $password1, $email, $role, $proyect) == true)
			die('1');
		die('wrong');
	}
}
					...
					<form method="post" action="" name="new-user">
						...
						<div class="select-holder">
							...
							<select name="nuser-role">
								...
								if($role == 1 || $role == 2 || $role == 3)
									echo '<option value="4">Usuario</option>';
								?>
							</select>
						</div>
						...
						<div class="select-holder">
							...
							if($_proys->count_proyects() == 0)
								echo '<select name="proyect" disabled><option value="no">You need to create a category first</option></select>';
							else{
								echo '<select name="proyect">';
