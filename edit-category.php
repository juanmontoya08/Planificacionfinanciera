...
if(isset($_POST['act'])) {
	if($_POST['act'] == '1') {
		if(!isset($_POST['catid']) || !isset($_POST['name']) || !isset($_POST['desc']))
			die('wrong');
		...
		if($_cats->edit_cat($catid, $name, $desc) == true)
			die('1');
		die('wrong');
	}
}
			...
			<div class="center">
				<div class="form">
					<form method="post" action="" name="edit-cat" data-id="<?php echo $cat->id; ?>">
						...
						<div class="ni-cont">
							<input type="text" name="ncat-name" class="ni" value="<?php echo $cat->nombre; ?>" />
						</div>
						<span class="ncat-desc-left">Descripción de categoría (<?php echo 400-strlen($cat->descrp); ?> caracteres restantes):</span><br />
						<div class="ni-cont">
							<textarea name="ncat-descrp" class="ni"><?php echo $cat->descripcion; ?></textarea>
						</div>
