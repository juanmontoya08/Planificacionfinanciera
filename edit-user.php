...
if(isset($_POST['act'])) {
	if($_POST['act'] == '1') {
		if(!isset($_POST['userid']) || !isset($_POST['name']) || !isset($_POST['email']) || !isset($_POST['role']) || !isset($_POST['proyect']))
			die('wrong');
		if($_POST['userid'] == '' || $_POST['name'] == '' || $_POST['email'] == '' || $_POST['role'] == '' || $_POST['proyect'] == '')
			die('wrong');
		...
		if($_users->edit_user($userid, $name, $email, $role, $proyect) == true)
			die('1');
		die('wrong');
	}
}
					...
					<form method="post" action="" name="edit-user" data-id="<?php echo $userd->id; ?>">
						...
						<div class="select-holder">
							...
							<select name="euser-role">
								...
								<option value="4"<?php if($userd->role == 4) echo ' selected'; ?>>Usuario</option>
							</select>
						</div>
						...
						<div class="select-holder">
							<i class="fa fa-caret-down"></i>
							<?php
							if($_proys->count_proyects() == 0)
								echo '<select name="euser-proyect" disabled><option value="no">You need to create a category first</option></select>';
							else{
								echo '<select name="euser-proyect">';
								$proys = $_proys->get_proys_dropdown();
								if ($userd->proyect != null) {
									echo "<option selected='selected' value=\"{$userd->proyect}\">".$_proys->get_proyect_name($userd->proyect)."</option>";
								}
								while($proyy = $proys->fetch_object()) {
									echo "<option value=\"{$proyy->id}\">{$proyy->nombre}</option>";
								}
								echo '</select>';
							}
							?>
